...
Function XFL_RegisterExtensions()
	; Check SKSE version
	If SKSE.GetVersionRelease() >= 46 ; 45
		Debug.Trace("EFF Notification: SKSE Loaded.")
		SKSEExtended = true
	Else
		Debug.Notification("EFF ERROR: SKSE version 1.7.1 not met, some features will be unavailable.")
		SKSEExtended = false
	...
	If SKSE.GetVersionRelease() >= 44 ; 45
		...
		RegisterForModEvent("XFL_System_OpenInventory", "XFL_OpenInventory")
		RegisterForModEvent("XFL_System_UpdatePanel", "XFL_UpdatePanel")
...
Function XFL_AddFollower(Form follower)
	...
	XFL_OutfitController.XFL_ConvertOutfit(FollowerActor)
	XFL_OutfitController.XFL_RemovePersistentRef(FollowerActor) ; Remove outfit persistence, we don't need it as long as they are with us
...
Function XFL_RemoveFollower(Form follower, Int iMessage = 0, Int iSayLine = 1)
	...
	If FollowerActor && FollowerActor.IsDead() == False
		...
		ObjectReference akInventory = XFL_FollowerInventories[i]
		If akInventory && XFL_UnclaimedItems
			akInventory.RemoveAllItems(XFL_UnclaimedItems, true, true)
		Endif
...
Function XFL_SendActionEvent(int akIdentifier, int akCmd, Form akForm1 = None, Form akForm2 = None, int aiValue1 = 0, int aiValue2 = 0)
	EFFPlugin plugin = XFL_GetPlugin(akIdentifier)
	if !plugin
...
Function XFL_OpenInventory(Form akRef)
	...
	If !FollowerActor
		Debug.Trace("EFF ERROR: OpenInventory bad input form")
		return
	Endif

	Int index = XFL_GetIndex(FollowerActor)
	If index != -1
		ObjectReference refInventory = XFL_FollowerInventories[index]
		If refInventory
			refInventory.Activate(Game.GetPlayer())
		Endif
	Endif
EndFunction
...
Function XFL_MoveToInventory(Form akRef, Form akForm, int aiCount)
	Actor FollowerActor = akRef as Actor
	If !FollowerActor
		Debug.Trace("EFF ERROR: MoveToInventory bad input form")
		return
	Endif
	...
	If index != -1
		ObjectReference refInventory = XFL_FollowerInventories[index]
		If refInventory
			If (akForm as ObjectReference) ; Item being placed is persistent ref, move it
				refInventory.AddItem(akForm, aiCount, true)
			Else ; Non-persist, move the item
				FollowerActor.RemoveItem(akForm, aiCount, true, refInventory)
			Endif
		EndIf
	Endif
EndFunction
...
Function XFL_UpdatePanel(Form akRef)
	Actor FollowerActor = akRef as Actor
	If !FollowerActor
		Debug.Trace("EFF ERROR: UpdatePanel bad input form")
		return
	Endif

	If APNLExtended
		If XFL_FollowerList.HasForm(FollowerActor)
			XFL_Panel.RemoveActors(FollowerActor)
			XFL_Panel.AddActors(FollowerActor)
		Endif
	Endif
